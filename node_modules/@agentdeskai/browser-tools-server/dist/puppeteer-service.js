...
let launchedBrowserWSEndpoint = null;
// Cleanup management
 ...
 */
export function configurePuppeteerService(config) {
...
async function getHeadlessBrowserInstance() {
    ...
    if (headlessBrowserInstance) {
        try {
            ...
            console.log(`Reusing existing headless browser with ${pages.length} pages`);
 ...
 */
async function setCustomBrowserExecutable(launchOptions) {
    ...
    if (currentConfig.customBrowserPaths &&
        ...
        for (const browser of preferredBrowsers) {
            if (currentConfig.customBrowserPaths[browser] &&
                ...
                console.log(`Using custom ${browser} path: ${launchOptions.executablePath}`);
                return;
            }
        }
    }
    ...
    try {
        if (detectedBrowserPath && fs.existsSync(detectedBrowserPath)) {
            ...
            console.log(`Using detected browser path: ${launchOptions.executablePath}`);
        }
    }
    catch (error) {
 ...
 */
async function findBrowserExecutablePath() {
    ...
    try {
        ...
        if (errorMessage.includes("No Chrome installations found") ||
            error?.code === "ERR_LAUNCHER_NOT_INSTALLED") {
    ...
    if (platform === "win32") {
            ...
            if (regOutput) {
                ...
                for (const chromePath of defaultChromePaths) {
                    if (fs.existsSync(chromePath)) {
                        console.log(`Found Chrome via BLBeacon registry hint: ${chromePath}`);
                        return chromePath;
                    }
                }
            }
        ...
        for (const browser of preferredBrowsers) {
            const paths = winBrowserPaths[browser] || [];
        ...
        // Check each browser in preferred order
        for (const browser of preferredBrowsers) {
            const paths = macBrowserPaths[browser] || [];
            for (const browserPath of paths) {
                if (fs.existsSync(browserPath)) {
                    console.log(`Found ${browser} at ${browserPath}`);
        ...
        // Check each browser in preferred order
        for (const browser of preferredBrowsers) {
            const commands = linuxBrowserCommands[browser] ||
                [];
            for (const cmd of commands) {
                try {
                    // Use more universal commands for Linux to find executables
                    // command -v works in most shells, fallback to which or type
                    const browserPath = execSync(`command -v ${cmd} || which ${cmd} || type -p ${cmd} 2>/dev/null`, { encoding: "utf8" }).trim();
    ...
    throw new Error(`No browser executable found for platform ${platform}. Please specify a custom browser path.`);
}
/**
    ...
    browser.on("disconnected", () => {
        ...
        setTimeout(() => {
            ...
            if (!headlessBrowserInstance) {
                ...
                console.log(`Skipping cleanup for ${userDataDir} as new browser instance is active`);
            }
...
export function scheduleBrowserCleanup() {
    ...
    if (headlessBrowserInstance) {
        console.log(`Scheduling browser cleanup in ${BROWSER_CLEANUP_TIMEOUT / 1000} seconds`);
 ...
 */
export async function connectToHeadlessBrowser(url, options = {}) {
    console.log(`Connecting to headless browser for ${url}${options.blockResources ? " (blocking non-essential resources)" : ""}`);
    try {
        // Validate URL format
        try {
            new URL(url);
        ...
        const port = parseInt(launchedBrowserWSEndpoint.split(":")[2].split("/")[0]);
        ...
        if (options.viewport) {
            ...
            console.log(`Set viewport to ${options.viewport.width}x${options.viewport.height}`);
        }
        ...
        if (options.emulateNetworkCondition) {
            ...
            console.log(`Emulating ${options.emulateNetworkCondition} network conditions`);
        }
        ...
        if (options.blockResources) {
            ...
            console.log(`Blocking resource types: ${resourceTypesToBlock.join(", ")}`);
        }
        ...
        if (options.waitForSelector) {
            try {
                ...
                console.warn(`Failed to find selector "${options.waitForSelector}": ${selectorError.message}`);
        ...
        throw new Error(`Failed to connect to headless browser: ${error instanceof Error ? error.message : String(error)}`);
    }
}
