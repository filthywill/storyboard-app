...
export async function runPerformanceAudit(url) {
    try {
        ...
        throw new Error(`Performance audit failed: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
...
const extractAIOptimizedData = (lhr, url) => {
    ...
    if (audits["largest-contentful-paint"]) {
        ...
        if (lcpElement && lcpElement.details) {
            const lcpDetails = lcpElement.details;
        ...
        if (lcpImageAudit && lcpImageAudit.details) {
            const lcpImageDetails = lcpImageAudit.details;
        ...
        if (!metric.element_url && lcp.details) {
            const lcpDirectDetails = lcp.details;
        ...
        if (largestImageAudit && largestImageAudit.details) {
            const imageDetails = largestImageAudit.details;
        ...
        if (!metric.element_url) {
            ...
            if (networkRequests && networkRequests.details) {
                const networkDetails = networkRequests.details;
                if (networkDetails.items && Array.isArray(networkDetails.items)) {
                    ...
                    const imageResources = networkDetails.items
                        ...
                        .sort((a, b) => Math.abs(a.endTime - lcpTime) - Math.abs(b.endTime - lcpTime));
    ...
    if (audits["render-blocking-resources"]) {
        ...
        const rbrDetails = rbrAudit.details;
    ...
    if (audits["uses-http2"]) {
        ...
        const http2Details = http2Audit.details;
        if (http2Details &&
            http2Details.items &&
    ...
    if (networkRequests && networkRequests.details) {
        const resourceDetails = networkRequests.details;
        if (resourceDetails.items && Array.isArray(resourceDetails.items)) {
            ...
            let totalSizeKb = 0;
            let jsCount = 0, cssCount = 0, imgCount = 0, fontCount = 0, otherCount = 0;
            ...
            if (thirdPartyAudit && thirdPartyAudit.details) {
                const thirdPartyDetails = thirdPartyAudit.details;
                if (thirdPartyDetails.items && Array.isArray(thirdPartyDetails.items)) {
                    thirdPartyDetails.items.forEach((item) => {
