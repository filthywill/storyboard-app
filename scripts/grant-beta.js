...
function printUsage() {
  console.log(`
Grant complimentary Pro access to one existing user.

Usage:
  npm run grant-beta -- user@example.com
...
function parseEmail(argv) {
  ...
  if (args.length !== 1) {
    throw new Error('Pass exactly one email address. Example: npm run grant-beta -- user@example.com');
  }
...
async function findAuthUserByEmail(supabase, email) {
  ...
  for (let page = 1; ; page += 1) {
    ...
    if (matches.length > 1) {
      throw new Error(`Found multiple Auth users for ${email}; refusing to grant access.`);
    }
...
async function grantComplimentaryPro(supabase, user) {
  ...
  const existed = subscriptions?.length === 1;
  const stripeManagedFields = getStripeManagedFields(subscriptions?.[0]);
  if (stripeManagedFields.length > 0) {
    throw new Error(
      `Existing subscription appears Stripe-managed (${stripeManagedFields.join(', ')}); refusing to modify it.`
    );
  }
  ...
  const { error: upsertError } = await supabase
    ...
    .upsert(
      {
        user_id: user.id,
        // CloudAccessService grants Pro exclusively for "active" or "trialing".
        status: 'active',
  ...
  if (upsertError) {
    throw new Error(`Could not grant complimentary Pro access: ${upsertError.message}`);
  }
...
async function main() {
  ...
  const existed = await grantComplimentaryPro(supabase, user);
  console.log(existed
    ? '✔ Existing subscription updated with complimentary Pro access'
    : '✔ New complimentary Pro subscription created');
}
